...
class RFClassificationPipeline:
    ...
    def load_model(
        ...
        print(f"  - Prediction threshold: {pipeline.best_threshold:.2f} ({threshold_source})")
        
        if best_threshold is not None and best_threshold != saved_threshold:
            print(f"    → Saved optimal: {saved_threshold:.2f}")
            print(f"    → User-specified: {best_threshold:.2f}")
        
        print(f"  - Prediction window: {pipeline.prediction_minutes} minutes")
        print(f"  - Target threshold: {pipeline.threshold}%")
        print(f"  - Number of features: {len(pipeline.feature_cols)}")
        print("=" * 70 + "\n")
